...
import { Users, Shield, Calendar, Clock, Edit2, ShieldAlert } from 'lucide-react';

export default function ManagerStaffPage() {
  const { user } = useAuth();
  const { users, counters, tables, updateUserProfile } = useApp();
  const { toast } = useToast();

  const [activeTab, setActiveTab] = useState<'managers' | 'captains' | 'shifts'>('managers');
  const [selectedStaffId, setSelectedStaffId] = useState<string | null>(null);

  // Filter staff by restaurant
  ...
  // Permission toggler for managers
  const handleTogglePermission = (mgrId: string, name: string) => {
    // In demo, we just simulate toggling permission flags
    toast({
      type: 'success',
  ...
  const handleAssignCounter = (captainId: string, counterId: string) => {
    ...
    // Update captain counterId and assigned tables based on counter tableRange
            ...
            <div className="overflow-x-auto border border-line rounded-lg">
              <table className="min-w-full divide-y divide-line text-left text-xs bg-bg-card">
                ...
                <tbody className="divide-y divide-line text-ink-soft font-medium">
                  {captains.map(capt => {
                    const currentCounter = restaurantCounters.find(c => c.id === capt.counterId);
                    return (
